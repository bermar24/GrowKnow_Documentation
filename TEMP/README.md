...
## Project dependencies, frameworks, and tools

### Automation

- **n8n** — workflow automation and scheduled ingestion.
- **Ollama** — local LLM used in the news filtering workflow.
- **LangChain n8n nodes** — AI-enabled nodes used by the automation flow.
- **Docker Compose** — runs the local n8n instance with persistent data.

### Database

- **SQLite** — local development database.
- **Postgres / Supabase** — production and hosted database option via `SUPABASE_DB_URL`.
- **dj-database-url** — parses the database URL in Django settings.

### Backend

- **Django 5.2.7** — backend web framework.
- **Django REST Framework** — API layer for news and tools endpoints.
- **django-cors-headers** — enables cross-origin access from the frontend.
- **whitenoise** — serves static files in production.
- **gunicorn** — production WSGI server.
- **python-dotenv** — loads local `.env` values during development.
- **psycopg2-binary** — PostgreSQL driver.
- **behave / behave-django** — BDD support for backend scenarios.
- **pytest / pytest-django / coverage** — testing and coverage tooling.
...
- **@supabase/supabase-js** — Supabase client for data access when enabled.
- **ESLint** — linting, with React/Vite-related plugins and TypeScript support.

### Testing

- **Django `TestCase` / `Client`** — backend API tests.
- **BDD scenarios (`behave`)** — behavior-driven acceptance tests.
- **`pytest` + `pytest-django`** — Python test execution for backend code.
- **Coverage reporting** — verifies how much code is exercised by tests.
